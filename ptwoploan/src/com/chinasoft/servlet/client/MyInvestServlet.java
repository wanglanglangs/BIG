...
public class MyInvestServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		//数据总行数
		int count = uis.queryCountMyInvest(account);
		//求总页数
		...
		//查询分页后的用户投资信息放入request中
		List<Invest> list = uis.queryMyInvest(account,(page-1)*every,every);
		request.setAttribute("page", page);
		request.setAttribute("myinvest", list);
		request.setAttribute("pageCount", pageCount);
		//请求转发到我的投资页面
		request.getRequestDispatcher("/client/myaccount/myinvest.jsp").forward(request, response);
	}
